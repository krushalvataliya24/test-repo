 ...
 @livewireScripts
 <script type="text/javascript" src="build/js/excel.js"></script>
  <!-- dataset -->
  <script>
   const data = {
  styles: {
...
const excel = new dhx.Spreadsheet("spreadsheet", {
  menu: false,
});
excel.parse(data);
...
document.addEventListener('DOMContentLoaded', function () {
  setTimeout(function () {
    const filterButton = document.querySelector('.dhx_toolbar-button[data-dhx-id="filter"]');
    if (filterButton) {
      filterButton.click();
    }
  }, 500);
...
let modalSpreadsheetInstance = null;

$('#add_excel').on('shown.bs.modal', function () {
  if (!modalSpreadsheetInstance) {
    modalSpreadsheetInstance = new dhx.Spreadsheet("spreadsheetModal", {
      menu: false,
    });
    modalSpreadsheetInstance.parse(data);

    setTimeout(() => {
      const filterButton = document.querySelector('#spreadsheetModal .dhx_toolbar-button[data-dhx-id="filter"]');
      if (filterButton) {
        filterButton.click();
      }
    }, 500);
